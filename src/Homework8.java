...
class Homework8 {
    public static void main(String[] args) {
        try {
            ...
            PrintStream ps = new PrintStream("fileio.out");

            int n = scn.nextInt();
            int m = scn.nextInt();
            HashMap<BaseStaff, java.lang.Integer> hashMap = new HashMap<BaseStaff, java.lang.Integer>();
            for (int i = 0; i < n; i++) {
            ...
            int maxCount = 0;
            BaseStaff best = null;
            for (Map.Entry<BaseStaff, java.lang.Integer> entry : hashMap.entrySet()) {
                if (entry.getValue() > maxCount) {
                    maxCount = entry.getValue();
                    best = entry.getKey();
                }
            }
            ps.println(best.toString());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
